...
hybrid_compare_g_type_method(LocationHybridPrivate *priv, GType g_type)
{
	...
	} else if (g_type == LOCATION_TYPE_GPS) {
		hybrid_set_current_method(priv, LOCATION_TYPE_GPS);
		return TRUE;
                       ...
                       gpointer self)
{
	...
	if (g_type == LOCATION_TYPE_GPS) {
		...
		LOCATION_LOGD("g_type is LOCATION_TYPE_MOCK");
	}
	...
	if (hybrid_compare_g_type_method(priv, g_type)) {
		...
		if (location_setting_get_int(VCONFKEY_LOCATION_MOCK_ENABLED) == 1 &&
			location_setting_get_int(VCONFKEY_LOCATION_MOCK_STATE) == VCONFKEY_LOCATION_POSITION_CONNECTED &&
			g_type == LOCATION_TYPE_MOCK) {

			if (priv->gps) g_object_get(priv->gps, "is_started", &gps_started, NULL);
			if (gps_started == TRUE) {
				LOCATION_LOGD("Calling GPS stop");
				ret = location_stop(priv->gps);
				if (ret != LOCATION_ERROR_NONE) {
					LOCATION_LOGW("Failed to stop GPS : [%d]", ret);
					return;
				}
			}
			...
			if (wps_started == TRUE) {
				...
				if (ret != LOCATION_ERROR_NONE) {
					LOCATION_LOGW("Failed to stop WPS : [%d]", ret);
					return;
				}
			}
		}
...
#if 0 // They are integrated to STATUS_CHANGED.
